...
for window in 2 1; do
  for lang in $@; do
    ...
    time java -Xmx144g -Djava.library.path=/usr/local/lib -cp bin/sp.jar com.statnlp.example.sp.main.SemTextExperimenter_Discriminative -thread $thread $config -lang $lang -neural -optim $optim -lr $lr -l2 $L2 -iter $iter -save-iter $saveiter -save-prefix $saveprefix -num-layer $num_layer -hidden $hidden -embedding $emb -embedding-size $embsize -window $window -dropout $dropout $pretrain $fixpre -neural-save-prefix $nnsaveprefix -noise-test > logs/"$saveprefix".out 2> logs/"$saveprefix".err
  done
done
